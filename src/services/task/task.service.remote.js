...
function moveTask(board, fromGroupId, toGroupId, fromIdx, toIdx) {
    const fromGroupIdx = board.groups?.findIndex(group => group.id === fromGroupId)
...
async function transferTask(sourceBoard, task, sourceGroupId, destBoard, newGroupId) {
    const isSameBoard = sourceBoard._id === destBoard._id
    
    // Find source group and task
    const sourceGroupIdx = sourceBoard.groups?.findIndex(g => g.id === sourceGroupId)
    if (sourceGroupIdx === -1) throw new Error(`Group with id ${sourceGroupId} not found`)
    
    const sourceGroup = sourceBoard.groups[sourceGroupIdx]
    if (!sourceGroup || !sourceGroup.tasks) throw new Error(`Group ${sourceGroupId} has no tasks`)
    
    const taskIdx = sourceGroup.tasks.findIndex(t => t.id === task.id)
    if (taskIdx === -1) throw new Error(`Task with id ${task.id} not found in group ${sourceGroupId}`)
    
    // Find destination group
    const destGroupIdx = destBoard.groups?.findIndex(g => g.id === newGroupId)
    if (destGroupIdx === -1) throw new Error(`Group with id ${newGroupId} not found in destination board`)
    ...
    // Add to destination
    const updatedDestBoard = isSameBoard 
        ? updatedSourceBoard
        : { ...destBoard }
    ...
    const finalDestBoard = {
        ...
        groups: updatedDestBoard.groups.map((g, i) => 
            i === destGroupIdx
                ? { ...g, tasks: [...(g.tasks || []), taskToMove] }
                : g
        )
    }
    
    return { sourceBoard: isSameBoard ? finalDestBoard : updatedSourceBoard, destBoard: finalDestBoard }
}

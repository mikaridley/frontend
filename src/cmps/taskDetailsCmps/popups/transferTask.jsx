...
export function TransferTask({ board, groupId, taskId, onClose, position }) {
  ...
  useEffect(() => {
    //update the selected board and group when the selected board id changes
  ...
  async function loadSelectedBoard(boardId, preserveGroupId = '') {
    //preserveGroupId: if new board has the same group, keep the same group
    try {
      const loadedBoard = await boardService.getById(boardId)
      setSelectedBoard(loadedBoard)
      if (
        preserveGroupId &&
        loadedBoard.groups?.some(g => g.id === preserveGroupId)
      ) {

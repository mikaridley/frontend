...
function setCoverBackgroundFromImage(imageUrl) {
    if (!imageUrl) return
    taskService.getDominantColor(imageUrl).then(color => {
...
export function TaskDetailsCover({ task, board, groupId, taskId, onTaskUpdate, onOpenPopup, attachments, boardId }) {
    ...
    const hasCoverProp = task && Object.prototype.hasOwnProperty.call(task, 'cover')
    ...
    const shouldFallbackToAttachment = !hasCoverProp && photoAttachments.length > 0
    ...
    if (cover && cover.kind !== 'photo') {
        ...
        // if you ever distinguish 'solid' vs 'gradiant', you can branch here
    ...
    return (
        <div className="task-details-cover" style={coverStyle}>
            ...
            {coverImageUrl && (
                <img
                    src={coverImageUrl}
                    style={{ width: '150px', height: '150px', objectFit: 'cover', justifySelf: 'center' }}
                    alt="Task cover"
